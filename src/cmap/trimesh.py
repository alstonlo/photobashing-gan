...
class TriMesh(object):
    ...
    def update_face_normals_and_areas(self):
        if self.__face_normals is None:
            self.__face_normals = zeros((len(self.faces), 3))
        if self.__face_areas is None:
            self.__face_areas = zeros(len(self.faces))
    ...
    def get_face_normals(self):
        if self.__face_normals is None:
            self.update_face_normals_and_areas()
    ...
    def update_vertex_normals(self):
        if self.__vertex_normals is None:
            self.__vertex_normals = zeros((len(self.vs), 3))
    ...
    def update_vertex_areas(self):
        if self.__vertex_areas is None:
            self.__vertex_areas = zeros(len(self.vs))
    ...
    def directed_edge2he_index(self, edge):
        ...
        if self.__directed_edge2he_index is None:
    ...
    def remove_vertex_indices(self, vertex_indices_to_remove):
        ...
        if 0 == len(vertex_indices_to_remove):
            return arange(len(self.vs))
    ...
    def remove_face_indices(self, face_indices_to_remove):
        ...
        if 0 == len(face_indices_to_remove):
            return arange(len(self.faces))
